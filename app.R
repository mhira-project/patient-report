...
ui <- dashboardPage(skin = "purple",
            ...
            Shiny.setInputValue('currentLang', CL);
                    ...
                    dashboardBody(
                      ...
                      fluidRow(
                        style = "border: 1px solid black; background-color: #ffffff; width: 800px",
                        p(disclaimer))
  ...
  observe({
    ...
    for (q in scalesFlt()$questionnaireVersionId %>% unique){
      ...
      local({ #https://gist.github.com/wch/5436415/
        ...
        questName = scalesFlt() %>%
          filter(questionnaireVersionId == my_q) %>%
          '[['("questionnaireShortName") %>% unique()
        
        description = scalesFlt() %>%
          filter(questionnaireVersionId == my_q) %>%
          '[['("description") %>% unique()
        ...
        output[[my_q]] <-  renderUI(
          div(
            ...
            br(),
            if(!is.na(description)){div(
              HTML(description),  # Render the HTML description using HTML()
              class = "description-box"  # CSS class for styling the box
            )},
            div(h3(transMatrix["figure", lang()])),
            if (!is_empty(plots)) {
              lapply(plots,
