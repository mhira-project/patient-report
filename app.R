  ...
  observe({
    ...
    for (q in scalesFlt()$questionnaireShortName %>% unique){
      ...
      local({ #https://gist.github.com/wch/5436415/
        ...
        int =   s %>% interpretTable(showScale = F)
        colnames(int) <- transMatrix[c("assessment","interpretation","recommendation", "warning"), input$currentLang]
        
        interpret =  int %>%
          datatable() %>% 
          formatStyle(valueColumns = transMatrix["warning", input$currentLang],
                      columns = c(transMatrix["warning", input$currentLang], transMatrix["assessment", input$currentLang]), 
                      #  target = 'row',
